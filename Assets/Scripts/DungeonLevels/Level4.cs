...
public class Level4 : MonoBehaviour
{
    ...
    private void begin()
    {
        ...
        grid.door(grid.grid[grid.getGridSizeX() - 1, grid.getGridSizeY() - 4], "vertical", exit);
        ...
        foreach (Node node in grid.grid)
        {
            if (node == grid.grid[grid.getGridSizeX() - 1, grid.getGridSizeY() - 4] && node.onTop == "Exit")
            {
                ...
                GameObject exitDoor = Instantiate(door, node.worldPosition + new Vector3(0, 0, -0.11f), Quaternion.identity, prefabManager.transform);
